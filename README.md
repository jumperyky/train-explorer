...
| ネットワーク | 路線 | 種別 |
| --- | --- | --- |
| 新幹線 | 東海道・山陽・東北・上越・北陸・九州・北海道・山形・秋田・西九州 | のぞみ／はやぶさ／かがやき／つばさ／こまち ほか |
| JR西日本 在来線 | 琵琶湖線・JR京都線・JR神戸線・湖西線・北陸本線 | 普通／快速／新快速／特急 |
| 近江鉄道 | 本線・多賀線・八日市線 | 普通 |
| 京阪電車 | 本線・鴨東線／石山坂本線／京津線 | 普通／急行／特急 |
| 京都市営地下鉄 | 烏丸線・東西線 | 普通 |
| 信楽高原鐵道 | 信楽線 | 普通 |
| 阪急電車 | 神戸本線・宝塚本線・京都本線 | 普通／急行／特急 |
| 近鉄 | 京都線・奈良線 | 普通／快速急行／特急 |

計 29路線・453駅・21車両。特急は「サンダーバード」「はるか」「らくラクびわこ」を
路線の種別として持たせています（走る路線がそれぞれ違うため、路線ごとに登録）。

新幹線は運転系統で持っています。北陸新幹線の正式な区間は高崎〜敦賀ですが、
列車はすべて東京から来るので、東京・上野・大宮を頭に付けた「東京〜敦賀」で
1路線にしています。
...
ルビ実装でつまずいた点と回避方法は
[docs/furigana-ruby-guide.md](docs/furigana-ruby-guide.md) にまとめてあります。
このプロジェクトに依存しない内容なので、他のアプリにそのままコピーできます。
...
路線の端でつながる先は `connections` に書きます。路線ページは1路線ぶんの駅しか
出さないので、そのままだと東海道新幹線は新大阪で行き止まりに見えます。書いておくと
路線図の端（と分岐する駅）に帯が出て、次の路線のページへそのまま進めます。

```ts
connections: [
  {
    lineId: "sanyo-shinkansen",   // つながる先
    stationId: "shin-osaka",      // つなぎ目。この路線の駅であること
    through: true,                // 直通なら true、乗りかえなら false
    note: "のぞみ・ひかりは そのまま 走《はし》っていくよ。",
  },
],
```

同じ駅を通る路線どうしの乗りかえは `linesAtStation` が駅の行に自動で出すので、
`connections` に書くのは「線路の続き」だけです。`check:data` が、つなぎ目の駅が
その路線にあるか・相手の路線から戻ってこられるかを検査します。

路線ごと増やすときは、駅一覧の取り込みから始められます。Wikipedia の路線記事の
駅一覧テーブルを読んで、`stations.ts` に貼れる形と `stationIds` の並びを出力します。

```bash
node scripts/fetch-line-stations.mjs "京阪本線" --prefix=keihan --minzoom=11
```

既存の駅と同名のものは「再利用」として報告します（同じ駅を二重に持たないため）。
記事名が別の駅にリダイレクトしている場合（`京阪石山駅` → `石山駅` など）は
`[要確認]` が出るので、読みとローマ字を手で直してください。
